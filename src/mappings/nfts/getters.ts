import {
  NftsSwapCancelledEvent,
  NftsSwapClaimedEvent,
  NftsSwapCreatedEvent,
  NftsTipSentEvent,
} from '../../types/events'
...
export function getCreateSwapEvent(ctx: Context): CreateSwapEvent {
  ...
  if (event.isV9370) {
    ...
    return {
      ...
      surcharge: price?.direction?.__kind,
      expiresAt: BigInt(deadline),
      target: createTarget(desiredCollection, desiredItem),
    }
  }
...
export function getClaimSwapEvent(ctx: Context): ClaimSwapEvent {
  ...
  if (event.isV9370) {
    ...
    return {
      ...
      expiresAt: BigInt(deadline),
      target: {
        ...createTarget(receivedCollection, receivedItem),
